"""Stage-1 screen v2.1 PHASE-2B scan + verdict (spec 01 §9; the once-only
rebuild culmination).

SCAN side stays on CURRENT (2026-07) GTFS -- candidates must be buildable
today (§9.4 asymmetry). Window predictors come from the SAME
screen_common_v21.compute_predictors_v21 as the fit side, at the "scan"
vintage (LODES 2022 / ACS 2023). Everything downstream of the inputs is the
v2.0 machinery carried over unchanged: index definition (same-exposure rebase),
route-cluster bootstrap, decision tripwire v2, shortlist-stability report,
output guardrails (no boardings field; two service levels only).

Adds, per §9:
  * the frozen 20-row v2.1 battery screen_battery_rows_v21 (loyo IN; sld OUT;
    genjobs_off / genjobs_leave_class_out redefined against the WAC term;
    window_10/window_15 host-shape-unit for criterion 3);
  * the §9.10 regime-split gate (pooled / pre-2020-only / interaction) with the
    BINDING downgrade rule;
  * the §9.1 b4_wrong_sign flag (set when l_genjobs is negative).

BOOTSTRAP NOTE (v2.1, stated). The §3.4 route-cluster bootstrap is carried over
(resample routes with replacement, refit, rescore ALL windows jointly). The
§3.4 WITHIN-REPLICATE ACS-MOE perturbation is NOT applied: the committed §9.2
block-apportionment input machinery (screen_common_v21, frozen phase-2a under
its no-fit hold) carries no ACS MOE, and the design-stage power check
pre-registered this omission as a stated stylization (outputs/
screen_power_check.json). This is an input-availability fact, not a tuning
choice; it removes one noise source, so criterion 1 is marginally MORE lenient
(stated, artifact `bootstrap`).

Artifact: outputs/screen_results_v21.json (NEW file -- does NOT overwrite the
v2.0 outputs/screen_results.json). Chart: outputs/screen_ranked_v21.png.
Deterministic write (sort_keys, indent=2, LF, floats 6dp, no timestamps; dual
fresh-process byte-identity is a gate).

    python -X utf8 scripts/screen_scan_v21.py
"""
import json
import math
import os
...
CONSUMED = (
    ...
    ("screen_battery_rows_v21", "FROZEN v2.1 20-row battery"),
...
def assemble_scan(data, cns2022, projs, buffer_mi, window_mi, step_mi):
    ...
    the scan vintage (block resolution). Also the fitted-route full-current-
    shape catchments (underservice diagnostic). Sorted route iteration;
    integer-step w0."""
...
# ---------------------------------------------------------------------------
# scoring design (windows / routes at a common service level)
# ---------------------------------------------------------------------------
def _b4_col(raw, cfg):
...
def score_design_v21(cols, cfg, svc, lengths, fe_year=None):
    """Design matrix for SCORING units at service `svc`. Year FE all 0 (a
    common FE cancels in the index) unless fe_year names one (the
    underservice diagnostic at fy2019). Interaction columns never score
    windows (regime-split is a fit-only diagnostic)."""
    n = len(cols["b1"])
    lengths = np.broadcast_to(np.asarray(lengths, float), (n,))
    names, fys = sf.col_order_v21(cfg)
    colmap = {
        "const": np.ones(n),
...
def _index_from_preds(win_pred, host_groups):
    ...
    (deterministic). Any common additive term cancels (svc/FE rank-inert)."""
...
def score(fit_rows, asm, cfg, svc, beta=None):
    """Point pipeline for one config: fit (unless beta given), score all
    current windows at standardized service, return index + fit artifacts."""
    y, X, names, groups, df = sf.design_matrix_v21(fit_rows, cfg)
    if beta is None:
        beta = sf.ols_beta(y, X)
    routes = sorted(set(df["route"]), key=sf._route_sort_key)
    wcols = _score_cols(asm["W"], cfg)
    Xw = score_design_v21(wcols, cfg, svc, asm["window_mi"])
    win_pred = Xw @ beta
    if cfg["offset"]:
...
def bootstrap(fit_rows, asm, cfg, svc, n_boot, seed, quiet=False):
    ...
    head = score(fit_rows, asm, cfg, svc)
...
def sign_fracs_only(fit_rows, cfg, n_boot, seed):
    """b1/b2 (and b4) bootstrap positive-sign fractions for a FIT-ONLY config
    (regime-split pre-2020 / interaction) -- no window scoring needed."""
    y, X, names, groups, df = sf.design_matrix_v21(fit_rows, cfg)
    routes = sorted(set(df["route"]), key=sf._route_sort_key)
    nR = len(routes)
...
def row_tie_bootstrap(fit_rows, asm, cfg, svc, n_boot, seed, nb_alpha=None,
                      drop_fy=None):
    ...
    holds NB2 alpha fixed and Fisher-scores beta per replicate."""
    rows = fit_rows[fit_rows["fy"] != drop_fy] if drop_fy else fit_rows
    head = score(rows, asm, cfg, svc)
    routes, nR, route_rows = _boot_setup(head)
    y, X = head["y"], head["X"]
    Xw = head["Xw"]
    nW = len(asm["W"]["window_id"])
...
# ---------------------------------------------------------------------------
def loo_battery(fit_rows, asm, svc, head):
    """Leave-one-ROUTE-out: OOS log errors (MALE, P90 for the underservice
    flag) + the window-ranking leverage screen (min Spearman rho vs
    headline)."""
    y, X = head["y"], head["X"]
    groups = head["groups"]
    routes = head["routes"]
...
def loyo_rhos(fit_rows, asm, svc, head):
    """Leave-one-YEAR-out over the 6 fit-panel FYs (the §9.3/§9.8 restored
    battery row -- vintage-matched X varies within route across years, so
    LOYO is now evidence, not a mechanical ~1). Returns {fy: rho} and the
    worst (min-rho) fy. NOTE: §9.8's '3 year-dropped refits' wording predates
    the §9.9 panel extension; leave-one-year-out over the EXTENDED 6-FY panel
    drops each of the 6 years (the fy2017 drop leaves a single-vintage X pair,
    stated in §9.8)."""
    out = {}
    for fy in val("screen_panel_ext_fys"):
        ...
        if rows["route"].nunique() < 3 or len(rows) <= head["X"].shape[1]:
            continue
        v = score(rows, asm, sf.BASE_CFG_V21, svc)
...
# ---------------------------------------------------------------------------
# sensitivity battery (frozen 20-row v2.1 list) + per-row stability contexts
# ---------------------------------------------------------------------------
...
def sensitivity_block(data, cns2022, cns_fit, fit, fit_projs, asm, head, svc,
                      tract_sets, groups_head, buffer_mi, window_mi, step_mi,
                      quiet=False):
    """Every frozen v2.1 battery row's rho + churn, plus the per-row stability
    contexts (each row's OWN tie-set bootstrap reruns from these). Returns
    (rows_in_registry_order, nb_info, ctxs)."""
    fit_rows = fit["rows"]
    rows, ctxs = [], {}
    ...
    for rid, buf in (("buffer_lo", band("buffer_mi")[0]),
        ...
        a2 = assemble_scan(data, cns2022, asm["projs"], buf, window_mi,
                           step_mi)
        v = score(f2["rows"], a2, sf.BASE_CFG_V21, svc)
        assert a2["W"]["window_id"] == hids
        rows.append(_row(rid, f"catchment buffer {buf} mi", hidx, hids,
                         v["index"]))
        ctxs[rid] = {"fit_rows": f2["rows"], "asm": a2, "cfg": sf.BASE_CFG_V21,
                     "svc": svc, "unit": "window_id"}
    ...
    for rid, wlen in (("window_10", band("screen_window_mi")[0]),
        ...
        a2 = assemble_scan(data, cns2022, asm["projs"], buffer_mi, wlen,
                           step_mi)
        v = score(fit_rows, a2, sf.BASE_CFG_V21, svc)
        rows.append(_length_row(rid, f"window length {wlen} mi", head, asm,
                                v, a2))
        ctxs[rid] = {"fit_rows": fit_rows, "asm": a2, "cfg": sf.BASE_CFG_V21,
                     "svc": svc, "unit": "host_shape"}
    # model-config perturbations on the headline assembly
    cfg_rows = (
        ("drop_fy2020", "drop FY2020 rows", {"drop_fy2020": True}),
        ("drop_rh", "fit without revenue hours (firewall)", {"use_rh": False}),
    ...
    for rid, label, over in cfg_rows:
        say(rid)
        cfg_v = dict(sf.BASE_CFG_V21, **over)
        v = score(fit_rows, asm, cfg_v, svc)
    ...
    for cns in naics:
        v = score(fit_rows, asm, dict(sf.BASE_CFG_V21, b4_ex_cns=cns), svc)
    ...
    ctxs["genjobs_leave_class_out"] = {
        "fit_rows": fit_rows, "asm": asm, "svc": svc, "unit": "window_id",
        "by_class": {cns: dict(sf.BASE_CFG_V21, b4_ex_cns=cns)
                     for cns in naics}}
    # NB2 estimator row (always fitted)
    say("nb_estimator")
    nb_beta, nb_alpha, nb_conv = sf.fit_nb2(
        head["df"]["boardings"].to_numpy(), head["y"], head["X"],
        head["beta"])
    v = score(fit_rows, asm, sf.BASE_CFG_V21, svc, beta=nb_beta)
    rows.append(_row("nb_estimator", "NB2 robustness estimator", hidx, hids,
                     v["index"], extra={"alpha": float(nb_alpha),
                                        "converged": bool(nb_conv)}))
    ctxs["nb_estimator"] = {"fit_rows": fit_rows, "asm": asm,
                            "cfg": sf.BASE_CFG_V21, "svc": svc,
                            "unit": "window_id", "nb_alpha": float(nb_alpha)}
    # svc p25/p75 (rank-inert by construction)
    for rid, s in (("svc_p25", band("screen_svc_std")[0]),
                   ("svc_p75", band("screen_svc_std")[1])):
        say(rid)
        v = score(fit_rows, asm, sf.BASE_CFG_V21, s)
        rows.append(_row(rid, f"standardized service {s} RVH/route-mi",
                         hidx, hids, v["index"],
                         note="single additive b3 term shifts every window "
                              "identically; index invariant by construction"))
        ctxs[rid] = {"fit_rows": fit_rows, "asm": asm, "cfg": sf.BASE_CFG_V21,
                     "svc": s, "unit": "window_id"}
    ...
    say("loyo")
    loyo_by_fy, worst_fy, loyo_rho = loyo_rhos(fit_rows, asm, svc, head)
    rows.append({"id": "loyo",
                 ...
                 "detail": {"rho": loyo_rho, "by_fy": loyo_by_fy,
                            ...
                            "note": "restored to the v2.1 battery (§9.3/§9.8) "
                                    "because vintage-matched X varies within "
                                    "route across years; leave-one-year-out "
                                    "over the extended 6-FY panel"}})
    ctxs["loyo"] = {"fit_rows": fit_rows, "asm": asm, "cfg": sf.BASE_CFG_V21,
                    "svc": svc, "unit": "window_id", "drop_fy": worst_fy}
    # freeze row order to the registry list (test asserts artifact == list)
    order = val("screen_battery_rows_v21")
...
def shortlist_stability(ctxs, sens, head_ties, head_tie_hosts, n_boot, seed,
                        quiet=False):
    """Every frozen battery row's OWN tie_with_cutoff set vs the margin-defined
    headline tie set (CRN per-row seed rule). Criterion-3 DUAL statistics:
    max_tie_churn_frac_window over WINDOW-UNIT rows, max_tie_churn_frac_
    hostshape over window_10/window_15. min_jaccard an all-rows report. Plus
    the stable core."""
    rows_order = val("screen_battery_rows_v21")
    sens_by_id = {r["id"]: r for r in sens}
    head_set_w, head_set_h = set(head_ties), set(head_tie_hosts)
    per_row, core = [], set(head_ties)

    def run_one(ctx, cfg):
        return row_tie_bootstrap(ctx["fit_rows"], ctx["asm"], cfg, ctx["svc"],
    ...
    return {
        ...
        "note": "per frozen v2.1 battery row (screen_battery_rows_v21): the "
                "row's OWN route-cluster bootstrap tie_with_cutoff set vs the "
                "MARGIN-DEFINED headline tie set. CRITERION 3 = DUAL "
...
# ---------------------------------------------------------------------------
# regime-split gate (§9.10)
# ---------------------------------------------------------------------------
def regime_split(fit_rows, n_boot, seed, pf_thr):
    """Three-way demand-block fit: POOLED (headline, all 6 FYs) / PRE-2020-ONLY
    (fy2017+fy2019) / FULL-PANEL-WITH-INTERACTION (post2020 x {b1, b2}). Reports
    the pre-2020 b1/b2 bootstrap sign fractions and the interaction
    coefficients. BINDING DOWNGRADE: pooled PASSES criterion 1 but pre-2020
    does NOT independently pass -> downgrade (regime_split_downgrade)."""
    pre_cfg = dict(sf.BASE_CFG_V21, panel="pre2020")
    int_cfg = dict(sf.BASE_CFG_V21, interaction=True)
    pre_pf, pre_nR, pre_n = sign_fracs_only(fit_rows, pre_cfg, n_boot, seed)
    # interaction point fit (coefficients + cluster SEs)
    y, X, names, groups, df = sf.design_matrix_v21(fit_rows, int_cfg)
...
# ---------------------------------------------------------------------------
# decision output v2.1 (tripwire v2 + regime-split downgrade)
# ---------------------------------------------------------------------------
def decision_block(pri, names, sens, windows, signs, n_boot, stability,
    ...
    # §9.10 binding downgrade: pooled passes crit 1 but pre-2020 does not
    ...
    regime_block = {
        ...
        "note": "§9.10 pre-registered gate (screen_regime_split): pooled / "
                "pre-2020-only (fy2017+fy2019) / full-panel-with-post2020-"
                "interaction. BINDING DOWNGRADE: pooled PASSES criterion 1 but "
                "pre-2020 does NOT independently pass the SAME "
                "screen_pos_frac_min bar -> ordinal_ok forced false, "
                "decision_format threshold_shortlist. Reuses the exact "
                "criterion-1 statistic; no new threshold",
    }
    ...
    return {
        ...
        "diagnostics": {"min_abs_t_demand": float(min_t),
                        ...
                        "note": "analytic cluster-robust min |t| over the "
                                ...
                                "b4_wrong_sign flag are diagnostics (§9.1)"},
        ...
        "note": "decision tripwire v2 on the v2.1 rebuild (spec 01 §5/§9.5). "
                "ordinal_ok requires criterion 1 (each demand coef b1/b2 "
                "bootstrap positive-sign fraction >= screen_pos_frac_min), "
                "criterion 2 (battery min Spearman rho >= "
                "screen_battery_rho_min over all 20 frozen rows), criterion 3 "
                "(DUAL: window-unit AND host-shape tie-churn both <= their "
...
def _underservice(fit_rows, data, asm, head, svc, loo_p90):
    """Host-route underservice gap (log points): predicted at STANDARDIZED
    service (fy2019 FE, svc_std x current route length) minus actual FY2019
    boardings, per fitted host route WITH a current shape and an fy2019 panel
    row; flagged beyond the LOO P90 abs error. Diagnostic-only (§3.2 caveat)."""
    fy19 = {r: b for r, b in zip(
        fit_rows[fit_rows["fy"] == "fy2019"]["route"],
        fit_rows[fit_rows["fy"] == "fy2019"]["boardings"])}
    cfg = sf.BASE_CFG_V21
    gap, flag = {}, {}
    for r in head["routes"]:
        ...
        X = score_design_v21(cols, cfg, svc, [proj.L], fe_year="fy2019")
        pred = float((X @ head["beta"])[0]) + (math.log(proj.L)
                                               if cfg["offset"] else 0.0)
        g = pred - math.log(float(fy19[r]))
        gap[r] = g
...
def make_chart(artifact, path):
    ...
    ax.set_title("Stage-1 DRM screen v2.1 — top 25 windows, P50 with P10-P90 "
                 "whiskers,\ncolored by host shape (ordinal_ok "
...
def build_artifact(n_boot=None, quiet=False):
    ...
    head = score(fit_rows, asm, sf.BASE_CFG_V21, svc)
    ...
    loo = loo_battery(fit_rows, asm, svc, head)
    boot = bootstrap(fit_rows, asm, sf.BASE_CFG_V21, svc, n_boot, seed,
                     quiet=quiet)
    ...
    if not quiet:
        print("sensitivity battery (20 frozen rows)...")
    sens, nb_info, ctxs = sensitivity_block(
        data, cns2022, cns_fit, fit, fit_projs, asm, head, svc, tract_sets,
        ...
        windows.append({
            ...
            "decomposition": {"demand": demand, "generator": generator,
                              "note": "grouped vs the lower-median fitted host "
                                      "route's best window; service/scale "
                                      "cancel at equal exposure"},
    ...
    flagged_routes = sorted(
        ...
        key=sf._route_sort_key)
    fit_diag = {
        "estimator": "log-OLS on log(annual boardings), cluster-robust SEs by "
                     "route; NB2 always fitted (nb_estimator row)",
        ...
        "b4_wrong_sign": {"flag": bool(b4_wrong_sign), "b4_genjobs_est": b4_est,
                          "note": "§9.1 pre-registered handling: a negative "
                                  "l_genjobs does NOT trip the demand-block "
                                  "criterion (b4 is outside the block) but "
                                  "raises this logged diagnostic -- a signal "
                                  "that b3/b5 may be absorbing attraction "
                                  "effects; setting it OBLIGATES a README "
                                  "known-issue entry before any gate memo "
        ...
        "v21_resid_decomposition": sf.resid_decomposition(fit_rows),
    ...
    run_id = nm.network_fingerprint({
        ...
        "vintages": VINTAGES, "special_generators": gens_cfg,
    ...
    artifact = {
        ...
        "bootstrap": {
            ...
            "acs_moe_note": "the §3.4 within-replicate ACS-MOE perturbation is "
                            "NOT applied: the committed §9.2 block-"
                            "apportionment input machinery (screen_common_v21, "
                            "frozen phase-2a under its no-fit hold) carries no "
                            "ACS MOE, and the design-stage power check "
                            "pre-registered this omission as a stated "
                            "stylization (screen_power_check.json). An "
        ...
        "notes": {
            "index": "screen_index = 100 * exp(window pred at standardized "
                     "service - baseline); baseline = lower-median over fitted "
                     "host routes of each route's own BEST window pred "
                     "(same-exposure rebase; ordinal only)",
...
def main(argv):
    ...
    out = os.path.join(OUT, "screen_results_v21.json")
    write_artifact(out, artifact)
    print(f"-> {out}  (run_id {artifact['run_id'][:16]})")
    png = os.path.join(OUT, "screen_ranked_v21.png")
    make_chart(artifact, png)
    print(f"-> {png}")
    d = artifact["fit_diagnostics"]
    co = d["coefficients"]
    print(f"\nfit N {d['n_route_years']} route-years / {d['n_routes']} "
          f"clusters; rows_by_fy {d['rows_by_fy']}")
    print("dropped shapeless:", d["dropped_route_years_shapeless"])
    print("dropped no-RVH:", d["dropped_route_years_no_rvh"])
    for k in ("b1_flows", "b2_zveh", "b3_rvh", "b4_genjobs", "b5_len"):
